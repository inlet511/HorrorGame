...
TSharedRef< UAssetBrowserDOC::FExtUDNParser > UAssetBrowserDOC::FExtUDNParser::Create( const TSharedPtr< FParserConfiguration >& ParserConfig, const FDocumentationStyle& Style )
{
	...
	if ( !FinalParserConfig.IsValid() )
	{
		struct Local
		{
			static void OpenLink( const FString& Link )
			{
				...
				{
					FNotificationInfo Info( NSLOCTEXT("FExtUDNParser", "FailedToOpenLink", "Failed to Open Link") );
	...
	TSharedRef< UAssetBrowserDOC::FExtUDNParser > Parser = MakeShareable( new UAssetBrowserDOC::FExtUDNParser( FinalParserConfig.ToSharedRef(), Style ) );
	Parser->Initialize();
	return Parser;
}

UAssetBrowserDOC::FExtUDNParser::FExtUDNParser( const TSharedRef< FParserConfiguration >& InConfiguration, const FDocumentationStyle& InStyle )
...
void UAssetBrowserDOC::FExtUDNParser::Initialize()
{
...
UAssetBrowserDOC::FExtUDNParser::~FExtUDNParser()
{
...
bool UAssetBrowserDOC::FExtUDNParser::LoadLink( const FString& Link, TArray<FString>& ContentLines )
{
...
bool UAssetBrowserDOC::FExtUDNParser::Parse(const FString& Link, TArray<FExcerpt>& OutExcerpts, FUDNPageMetadata& OutMetadata)
{
	FMessageLog UDNParserLog(UDNParseErrorLog);
...
bool UAssetBrowserDOC::FExtUDNParser::GetExcerptContent( const FString& Link, FExcerpt& Excerpt, bool bInSimpleText/* = false*/ )
{
	FMessageLog UDNParserLog(UDNParseErrorLog);
	...
	if ( LoadLink( Link, ContentLines ) )
	{
		bool bSimpleTextBackup = bSimpleText;
		bSimpleText = bInSimpleText;
		Excerpt.Content = GenerateExcerptContent( Link, Excerpt, ContentLines, Excerpt.LineNumber);
		bSimpleText = bSimpleTextBackup;
		return true;
	}
	else
	{
		...
		UDNParserLog.Error(FText::Format(LOCTEXT("GeneralExcerptError", "Generating a Widget for document '{0}' Excerpt '{1}' failed."), FText::FromString( FExtDocumentationLink::ToSourcePath( Link ) ), FText::FromString(Excerpt.Name) ));
	}

	return false;
}

void UAssetBrowserDOC::FExtUDNParser::SetWrapAt( TAttribute<float> InWrapAt )
{
	WrapAt = InWrapAt;
}

int32 UAssetBrowserDOC::FExtUDNParser::FTokenConfiguration::CalculatedExpectedContentStrings()
{
...
TSharedPtr<FSlateDynamicImageBrush> UAssetBrowserDOC::FExtUDNParser::GetDynamicBrushFromImagePath(FString Filename)
{
	FName BrushName( *Filename );

	if (FPaths::GetExtension(Filename) == TEXT("png") || FPaths::GetExtension(Filename) == TEXT("jpg"))
	{
...
FString UAssetBrowserDOC::FExtUDNParser::ConvertSymbolIntoAString(const FUDNToken& Token)
{
...
FString UAssetBrowserDOC::FExtUDNParser::ConvertSymbolsIntoAString(const TArray<FUDNToken>& TokenList, int32 StartingAfterIndex)
{
...
bool UAssetBrowserDOC::FExtUDNParser::ParseLineIntoSymbols(int32 LineNumber, const FString& Line, TArray<FUDNToken>& SymbolList)
{
	if (!Line.IsEmpty())
	{
		FString ChoppedLine;
...
UAssetBrowserDOC::FUDNLine UAssetBrowserDOC::FExtUDNParser::ParseLineIntoUDNContent(int32 LineNumber, const FString& Line)
{
	FMessageLog UDNParserLog(UDNParseErrorLog);
...
void UAssetBrowserDOC::FExtUDNParser::AppendExcerpt(TSharedPtr<SVerticalBox> Box, TSharedRef<SWidget> Content)
{
	Box->AddSlot()
	.AutoHeight()
...
void UAssetBrowserDOC::FExtUDNParser::AddContentToExcerpt(TSharedPtr<SVerticalBox> Box, const FString& ContentSource, FExcerpt& Excerpt)
{
	if ( !ContentSource.IsEmpty() )
	{
		const ISlateStyle& DocumentationStyle = FExtDocumentationStyle::Get();

		AppendExcerpt(Box,
			SNew(STextBlock)
			.Text(FText::FromString(ContentSource))
			.TextStyle(DocumentationStyle, Style.ContentStyleName)
			.WrapTextAt(WrapAt)
		);
		...
		if (bSimpleText)
		{
			Excerpt.RichText += FString::Printf(TEXT("%s"), *ContentSource);
		}
		else
		{
...
TSharedRef< SWidget > UAssetBrowserDOC::FExtUDNParser::GenerateExcerptContent( const FString& InLink, FExcerpt& Excerpt, const TArray<FString>& ContentLines, int32 StartingLineIndex )
{
	FMessageLog UDNParserLog(UDNParseErrorLog);

	const FString SourcePath = FExtDocumentationLink::ToSourcePath( InLink );
		...
		if ( VariableName.IsEmpty() )
		{
			...
			case FUDNLine::BoldContent:
				...
				AppendExcerpt(Box,
					...
					.TextStyle(DocumentationStyle, Style.BoldContentStyleName)
					);

				AddLineSeperator(Excerpt);
				if (bSimpleText)
					Excerpt.RichText += FString::Printf(TEXT("%s"), *Line.AdditionalContent[0]);
				else
			...
			case FUDNLine::HorizontalRule:
				...
				Box->AddSlot()
					...
					[
						...
						[
							SNew(SSeparator)
							.SeparatorImage(FExtDocumentationStyle::Get().GetBrush(Style.SeparatorStyleName))
						]
					];
			...
			case FUDNLine::Header1:
				...
				AppendExcerpt(Box,
					SNew(STextBlock)
					.Text(FText::FromString(Line.AdditionalContent[0]))
					.TextStyle(DocumentationStyle, Style.Header1StyleName)
					);
			...
			case FUDNLine::Header2:
				...
				AppendExcerpt(Box,
					SNew(STextBlock)
					.Text(FText::FromString(Line.AdditionalContent[0]))
					.TextStyle(DocumentationStyle, Style.Header2StyleName)
					);
			...
			case FUDNLine::Link:
				...
				AppendExcerpt(Box,
					SNew(SHyperlink)
					.Text(FText::FromString(Line.AdditionalContent[0]))
					.TextStyle(DocumentationStyle, Style.HyperlinkTextStyleName)
					.UnderlineStyle(DocumentationStyle, Style.HyperlinkButtonStyleName)
					.OnNavigate( this, &UAssetBrowserDOC::FExtUDNParser::HandleHyperlinkNavigate, Line.AdditionalContent[1])
					);
			...
			case FUDNLine::ImageLink:
				...
				if (DynamicBrush.IsValid())
				{
					...
					AppendExcerpt(Box,
						...
						.OnClicked( FOnClicked::CreateSP( this, &UAssetBrowserDOC::FExtUDNParser::OnImageLinkClicked, Line.AdditionalContent[2] ) )
						[
							SNew( SImage )
							.Image(DynamicBrush.Get())
							.ToolTipText(FText::FromString(Line.AdditionalContent[0]))
...
bool UAssetBrowserDOC::FExtUDNParser::ParseSymbols(const FString& Link, const TArray<FString>& ContentLines, const FString& FullPath, TArray<FExcerpt>& OutExcerpts, FUDNPageMetadata& OutMetadata)
{
	FMessageLog UDNParserLog(UDNParseErrorLog);
...
FReply UAssetBrowserDOC::FExtUDNParser::OnImageLinkClicked( FString AdditionalContent )
{
...
void UAssetBrowserDOC::FExtUDNParser::HandleHyperlinkNavigate( FString AdditionalContent )
{
	NavigateToLink( AdditionalContent );
}

void UAssetBrowserDOC::FExtUDNParser::NavigateToLink( FString AdditionalContent )
{
	static const FString DocLinkSpecifier( TEXT( "DOCLINK:" ) );
	static const FString TutorialLinkSpecifier( TEXT( "TUTORIALLINK:" ) );
	static const FString HttpLinkSPecifier( TEXT( "http://" ) );
...
bool UAssetBrowserDOC::FExtUDNParser::ParseCodeLink(FString &InternalLink)
{
...
bool UAssetBrowserDOC::FExtUDNParser::ParseAssetLink(FString &InternalLink)
{
