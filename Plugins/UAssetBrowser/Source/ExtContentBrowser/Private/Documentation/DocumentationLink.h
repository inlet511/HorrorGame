...
public: 
	...
	static FString ToSourcePath(const FString& Link, const FCultureRef& Culture, const FString& BasePath = FString())
	{
		...
		static FString DocumentationPath = IPluginManager::Get().FindPlugin(DocumentationHostPluginName)->GetBaseDir() / TEXT("Resources/Documentation/");
		const FString FullDirectoryPath = DocumentationPath + TEXT( "Source" ) + Path + "/";
