...
struct FExtConfigUtil
{
	...
	static bool LoadPathColors(const FString& InAssetContentRoot, const FString& InConfigDir, TMap<FName, FLinearColor>& OutPathColors, bool bRemapPackagePathToFilePath = true)
	{
		...
		{
			if (const FConfigSection* Sec = ConfigFile.FindSection(TEXT("PathColor")))
			{
...
FTopLevelAssetPath FExtAssetData::GetAssetClassPath() const
{
	return FTopLevelAssetPath(AssetClass.ToString());
}
...
namespace FExtAssetRegistryUtils
{
	bool RunAssetThroughFilter_Unchecked(const FExtAssetData& AssetData, const FARCompiledFilter& Filter, const bool bPassFilterValue)
	{
		// Package Names
		...
		if (Filter.PackagePaths.Num() > 0)
		{
			...
			for (const auto& Path : Filter.PackagePaths)
			{
				if (AssetData.GetFolderPath().StartsWith(Path.ToString()))
				{
		...
		if (Filter.ClassPaths.Num() > 0)
		{
			...
			for (const auto& Path : Filter.ClassPaths)
			{
				if (AssetData.AssetClass == Path.GetAssetName() || AssetData.AssetClass == FName(Path.GetPackageName().ToString() + "." + Path.GetAssetName().ToString()))
				{
					bPassesClasses = true;
					break;
				}
	...
	void RunAssetsThroughFilter(TArray<FExtAssetData>& AssetDataList, const FARFilter& Filter)
	{
		if (Filter.IsEmpty())
		{
			return;
		...
		const bool bPassFilterValue = true;// FilterMode == EFilterMode::Inclusive;

		AssetDataList.RemoveAll([&CompiledFilter, bPassFilterValue](const FExtAssetData& AssetData)
			{
...
{
	FExtAssetRegistryUtils::RunAssetsThroughFilter(AssetDataList, Filter);
}

bool FExtAssetRegistry::GetDependencies(const FExtAssetIdentifier& AssetIdentifier, TArray<FExtAssetIdentifier>& OutDependencies, UE::AssetRegistry::EDependencyCategory InDependencyType /*= UE::AssetRegistry::EDependencyCategory::All*/, const UE::AssetRegistry::FDependencyQuery& Flags)
{
	FName PackageName = AssetIdentifier.PackageName;
	TArray<FName> Dependencies;
	if (GetDependencies(PackageName, Dependencies, InDependencyType, Flags))
	{
...
bool FExtAssetRegistry::GetDependencies(FName PackageName, TArray<FName>& OutDependencies, UE::AssetRegistry::EDependencyCategory InDependencyType /*= UE::AssetRegistry::EDependencyCategory::Package*/, const UE::AssetRegistry::FDependencyQuery& Flags)
{
	// If no specific requirement flags (NoRequirements), include none
	bool bShowNone = (Flags.Required == UE::AssetRegistry::EDependencyProperty::None);
	if (bShowNone)
	{
		return false;
	}
	...
	if (AssetDataList.Num() > 0)
	{
		...
		if (AssetData.IsValid() && AssetData.HasContentRoot())
		{
			...
			// Check which flags are set to determine what to include
			bool bHardRequired = !!(Flags.Required & UE::AssetRegistry::EDependencyProperty::Hard);
			bool bSoftRequired = !!(Flags.Excluded & UE::AssetRegistry::EDependencyProperty::Hard);

			// If no specific requirement flags (Game), include none
			bool bShowBoth = (Flags.Required == UE::AssetRegistry::EDependencyProperty::Game) && !(Flags.Excluded & UE::AssetRegistry::EDependencyProperty::Hard);

			if (bShowBoth || bHardRequired)
			{
				Dependencies.Append(AssetData.HardDependentPackages);
			}

			if (bShowBoth || bSoftRequired)
			{
...
bool FExtAssetRegistry::GetDependencies(FName PackageName, TArray<FExtAssetDependencyNode>& OutDependencies, UE::AssetRegistry::EDependencyCategory Category /*= UE::AssetRegistry::EDependencyCategory::All*/, const UE::AssetRegistry::FDependencyQuery& Flags /*= UE::AssetRegistry::FDependencyQuery()*/)
{
	// If no specific requirement flags (NoRequirements), include none
	bool bShowNone = (Flags.Required == UE::AssetRegistry::EDependencyProperty::None);
	if (bShowNone)
	...
	if (AssetDataList.Num() > 0)
	{
		...
		if (AssetData.IsValid() && AssetData.HasContentRoot())
		{
			...
			*/
			...
			// todo: add editor only support
			//bool bEditorOnly = !!(Flags.Excluded & UE::AssetRegistry::EDependencyProperty::Game);

			// If ask for Game requirement without exclude flags, include both
			bool bShowBoth = (Flags.Required == UE::AssetRegistry::EDependencyProperty::Game) && !(Flags.Excluded & UE::AssetRegistry::EDependencyProperty::Hard);

			if (bShowBoth || bHardRequired)
			{
				for (const FName& HardPackge : AssetData.HardDependentPackages)
				{
					OutDependencies.Add(FExtAssetDependencyNode{ HardPackge , EDependencyNodeStatus::Valid, false});
				}
			}

			if (bShowBoth || bSoftRequired)
			{
				for (const FName& SoftPackage : AssetData.SoftReferencesList)
				{
					OutDependencies.Add(FExtAssetDependencyNode{ SoftPackage, EDependencyNodeStatus::Valid, true });
				}
			}
			...
			for (FExtAssetDependencyNode& Depend : OutDependencies)
			{
				FString RemappedFilePath;
				if (FExtAssetDataUtil::RemapGamePackageToFullFilePath(GameRoot, Depend.PackageName.ToString(), AssetContentDir, RemappedFilePath))
				{
					FString RemappedPackageName = FExtAssetData::ConvertFilePathToTempPackageName(RemappedFilePath);
					...
					Depend.PackageName = *RemappedPackageName;
				}
			}
...
bool FExtAssetRegistry::GetReferencers(const FExtAssetIdentifier& AssetIdentifier, TArray<FExtAssetIdentifier>& OutReferencers, UE::AssetRegistry::EDependencyCategory InDependencyType /*= UE::AssetRegistry::EDependencyCategory::All*/, const UE::AssetRegistry::FDependencyQuery& Flags)
{
	FName PackageName = AssetIdentifier.PackageName;
	TArray<FName> Referencers;
	if (GetReferencers(PackageName, Referencers, InDependencyType, Flags))
	{
		for (const FName& ReferencerPackage : Referencers)
		{
			OutReferencers.Add(ReferencerPackage);
		}
		return true;
	}
	return false;
...
//todo: for referencer lookup, current implementation is expensive (will go through all cached assets for each lookup)
bool FExtAssetRegistry::GetReferencers(FName PackageName, TArray<FName>& OutReferencers, UE::AssetRegistry::EDependencyCategory InDependencyType /*= UE::AssetRegistry::EDependencyCategory::Package*/, const UE::AssetRegistry::FDependencyQuery& Flags)
{
	// Scan through all cached assets to find which ones reference the given package
	// Check which flags are set to determine what to include
	bool bHardRequired = !!(Flags.Required & UE::AssetRegistry::EDependencyProperty::Hard);
	bool bSoftRequired = !!(Flags.Excluded & UE::AssetRegistry::EDependencyProperty::Hard);

	// If no specific requirement flags (NoRequirements), include both
	bool bNoSpecificRequirement = (Flags.Required == UE::AssetRegistry::EDependencyProperty::None);

	// Iterate through all cached assets
	for (const auto& CachedAssetPair : State.CachedAssetsByFilePath)
	{
		FExtAssetData* CachedAsset = CachedAssetPair.Value;
		if (CachedAsset && CachedAsset->IsValid())
		{
			// Check if this asset references the target package
			bool bFoundReference = false;

			// Check hard dependencies if required
			if (bNoSpecificRequirement || bHardRequired)
			{
				for (const FName& HardDep : CachedAsset->HardDependentPackages)
				{
					if (HardDep == PackageName)
					{
			...
			// Check soft references if required and not already found
			if (!bFoundReference && (bNoSpecificRequirement || bSoftRequired))
			{
				for (const FName& SoftRef : CachedAsset->SoftReferencesList)
				{
					if (SoftRef == PackageName)
					{
						bFoundReference = true;
						break;
					}
			...
			if (bFoundReference)
			{
				// Add the referencing asset's package name to the list
				if (CachedAsset->PackageName != NAME_None)
				{
					// Check if it's already in the output to avoid duplicates
					if (!OutReferencers.Contains(CachedAsset->PackageName))
					{
						OutReferencers.Add(CachedAsset->PackageName);
					}
				}
			}
		}
...
void FExtAssetRegistry::CacheAssetsAsync(const struct FARFilter& InFilter)
{
	...
	double PrepareAssetsTime = FPlatformTime::Seconds();
	...
	PrepareAssetsTime = FPlatformTime::Seconds() - PrepareAssetsTime;

	StartOrCancelAssetGathering(ToParseAssets.Array(), FilteredAssetPaths.Num());

	ECB_LOG(Display, TEXT("CacheAssetsAsync completed in %0.4f seconds. Kick off %d assets to gathering. (Prepare in %0.4f secons)"), FPlatformTime::Seconds() - GetAssetsStartTime, ToParseAssets.Num(), PrepareAssetsTime);
}
...
FName FExtCollectionUtil::GetOrCreateCollection(FName InCollectionName, bool bUniqueCollection)
{
	...
	if (bUniqueCollection)
	{
		...
		if (!CollectionManager.GetProjectCollectionContainer()->CollectionExists(CollectionName, ShareType))
		{
			if (CollectionManager.GetProjectCollectionContainer()->CreateCollection(CollectionName, ShareType, ECollectionStorageMode::Static))
			{
		...
		CollectionManager.GetProjectCollectionContainer()->CreateUniqueCollectionName(CollectionName, ShareType, UniqueCollectionName);
		if (CollectionManager.GetProjectCollectionContainer()->CreateCollection(UniqueCollectionName, ShareType, ECollectionStorageMode::Static))
		{
...
bool FExtCollectionUtil::AddAssetsToCollection(const TArray<FAssetData>& InAssets, FName InCollectionName, bool bUniqueCollection)
{
	...
	if (CollectionManager.GetProjectCollectionContainer()->AddToCollection(CollectionName, ShareType, ObjectPaths, &NumAdded))
	{
		//if (NumAdded > 0)
		{
			return CollectionManager.GetProjectCollectionContainer()->SaveCollection(CollectionName, ShareType);
		}
	}
	return false;
}
	...
	void ImportAssets(const TArray<FExtAssetData>& InAssetDatas)
	{
		...
		if (ImportSetting.bSyncAssetsInContentBrowser)
		{
				...
				&& SuccessImportedMainAssets.Num() > 0)
			{
				...
				{
					const FExtAssetData& MainAsset = InAssetDatas[0];
					FTSTicker::GetCoreTicker().AddTicker(
						FTickerDelegate::CreateLambda([MainAsset](float DeltaTime) -> bool
							{
								FExtAssetDataUtil::SyncToContentBrowser(MainAsset);
								return false;
							}),
						0.5f // Delay time (0.0f for next frame)
					);
				...
				if (bResetOrigLoaders)
				{
					...
					for (UPackage* OrigPackage : OrigPackages)
					{
						if (OrigPackage)
						{
							...
							OrigPackage->GetMetaData().RemoveMetaDataOutsidePackage(OrigPackage);
						}
					}
				}
			}
...
uint32 FExtAssetGatherer::Run()
{
	while (!bRequestToStop)
	{
		...
		FName LocalFilePathToGather;
		{
			...
			{
				LocalFilePathToGather = GatherBatch.Pop(EAllowShrinking::No);
			}
		}
...
namespace CachDBFileUtil
{
	bool SaveBufferToFile(const FString& FullFilePath, FBufferArchive& ToBinary)
	{
		...
		if (FFileHelper::SaveArrayToFile(ToBinary, *FullFilePath))
		{
			ToBinary.FlushCache();
			ToBinary.Empty();
			ECB_LOG(Display, TEXT("[SaveBufferToFile] success!"));
			return true;
		}
		// Free Binary Array
		ToBinary.FlushCache();
		ToBinary.Empty();
		ECB_LOG(Display, TEXT("[SaveBufferToFile] failed!"));
		return false;
	}
}
