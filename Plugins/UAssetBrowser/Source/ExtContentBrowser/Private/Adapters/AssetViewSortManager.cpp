...
public:
	...
	/** Sort function */
	FORCEINLINE bool operator()(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const
	{
...
protected:
	...
	FORCEINLINE virtual bool Compare(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const { return CompareNext(A, B); }

private:
	/** Compare the folders of both assets */
	FORCEINLINE bool CompareFolderFirst(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B, bool& bComparisonResult) const
	{
		if (A->GetType() == EExtAssetItemType::Folder)
		{
			if (B->GetType() == EExtAssetItemType::Folder)
			{
				const FText& ValueA = StaticCastSharedPtr<FExtAssetViewFolder>(A)->FolderName;
				const FText& ValueB = StaticCastSharedPtr<FExtAssetViewFolder>(B)->FolderName;
		...
		else if (B->GetType() == EExtAssetItemType::Folder)
		{
			bComparisonResult = !bAscending;
			return true;
		}
	...
	FORCEINLINE bool CompareNext(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const
	{
...
protected:
	FORCEINLINE virtual bool Compare(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const override
	{
		int32 Result = FCStringWide::Strcmp(StaticCastSharedPtr<FExtAssetViewAsset>(A)->FirstFewAssetNameCharacters, StaticCastSharedPtr<FExtAssetViewAsset>(B)->FirstFewAssetNameCharacters);
		if (Result == 0)
		{
			const FName& ValueA = StaticCastSharedPtr<FExtAssetViewAsset>(A)->Data.AssetName;
			const FName& ValueB = StaticCastSharedPtr<FExtAssetViewAsset>(B)->Data.AssetName;
...
protected:
	FORCEINLINE virtual bool Compare(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const override
	{
		const FName& ValueA = StaticCastSharedPtr<FExtAssetViewAsset>(A)->Data.AssetClass;
		const FName& ValueB = StaticCastSharedPtr<FExtAssetViewAsset>(B)->Data.AssetClass;
...
protected:
	FORCEINLINE virtual bool Compare(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const override
	{
		const FName& ValueA = StaticCastSharedPtr<FExtAssetViewAsset>(A)->Data.PackageFilePath;
		const FName& ValueB = StaticCastSharedPtr<FExtAssetViewAsset>(B)->Data.PackageFilePath;
		const int32 Result = ValueA.Compare(ValueB);
		if (Result < 0)
		{
			return bAscending;
...
protected:
	FORCEINLINE virtual bool Compare(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const override
	{
		...
		FString Value1;
		const bool bFoundValue1 = bAscending ? StaticCastSharedPtr<FExtAssetViewAsset>(A)->GetTagValue(Tag, Value1) : StaticCastSharedPtr<FExtAssetViewAsset>(B)->GetTagValue(Tag, Value1);
		if (!bFoundValue1)
		{
			return true;
		}
		...
		FString Value2;
		const bool bFoundValue2 = bAscending ? StaticCastSharedPtr<FExtAssetViewAsset>(B)->GetTagValue(Tag, Value2) : StaticCastSharedPtr<FExtAssetViewAsset>(A)->GetTagValue(Tag, Value2);
		if (!bFoundValue2)
		{
			return false;
		}
...
protected:
	FORCEINLINE virtual bool Compare(const TSharedPtr<FExtAssetViewItem>& A, const TSharedPtr<FExtAssetViewItem>& B) const override
	{
		// Depending if we're sorting ascending or descending it's quicker to flip the compares incase tags are missing
		FString Value1;
		const bool bHasFoundValue1 = bAscending ? StaticCastSharedPtr<FExtAssetViewAsset>(A)->GetTagValue(Tag, Value1) : StaticCastSharedPtr<FExtAssetViewAsset>(B)->GetTagValue(Tag, Value1);
		if (!bHasFoundValue1)
		{
			return true;
		}
		...
		FString Value2;
		const bool bHasFoundValue2 = bAscending ? StaticCastSharedPtr<FExtAssetViewAsset>(B)->GetTagValue(Tag, Value2) : StaticCastSharedPtr<FExtAssetViewAsset>(A)->GetTagValue(Tag, Value2);
		if (!bHasFoundValue2)
		{
			return false;
		}
...
bool FAssetViewSortManager::FindAndRefreshCustomColumn(TArray<TSharedPtr<FExtAssetViewItem>>& AssetItems, FName ColumnName, const TArray<FAssetViewCustomColumn>& CustomColumns, UObject::FAssetRegistryTag::ETagType& TagType) const
{
	...
	for (const FAssetViewCustomColumn& Column : CustomColumns)
	{
		if (Column.ColumnName == ColumnName)
		{
			...
			for (TSharedPtr<struct FExtAssetViewItem> AssetItem : AssetItems)
			{
				...
				FExtAssetViewAsset* Asset = StaticCastSharedPtr<FExtAssetViewAsset>(AssetItem).Get();
#if ECB_LEGACY
...
void FAssetViewSortManager::SortList(TArray<TSharedPtr<FExtAssetViewItem>>& AssetItems, const FName& MajorityAssetType, const TArray<FAssetViewCustomColumn>& CustomColumns) const
{
	...
	for (int32 PriorityIdx = 0; PriorityIdx < EColumnSortPriority::Max; PriorityIdx++)
	{
		...
		else
		{
			...
			{
				UClass* Class = FindObject<UClass>(nullptr, *MajorityAssetType.ToString());
				if (Class)
				{
...
void FAssetViewSortManager::ExportColumnsToCSV(TArray<TSharedPtr<struct FExtAssetViewItem>>& AssetItems, TArray<FName>& ColumnList, const TArray<FAssetViewCustomColumn>& CustomColumns, FString& OutString) const
{
	...
	for (TSharedPtr<struct FExtAssetViewItem> AssetItem : AssetItems)
	{
		...
		for (FName Column : ColumnList)
		{
			...
			{
				ValueString = Asset->Data.PackageFilePath.ToString();
			}
			else
			{
