...
public:
		...
		, TagKeyName("Tag")
	{
		if (FCollectionManagerModule::IsModuleAvailable())
		{
			...
			CollectionManagerModule.Get().GetProjectCollectionContainer()->GetCollections(AvailableCollections);
			...
			for (const FCollectionNameType& AvailableCollection : AvailableCollections)
			{
				...
				CollectionManagerModule.Get().GetProjectCollectionContainer()->GetCollectionStorageMode(AvailableCollection.Name, AvailableCollection.Type, StorageMode);
			...
			if (TextFilterUtils::TestBasicStringExpression(DynamicCollectionNameStr, InValue, InTextComparisonMode))
			{
				...
				if (!bCollectionAlreadyProcessed)
				{
					...
					if (FCollectionManagerModule::IsModuleAvailable())
					{
						...
						bool bUnused = false;
						CollectionManagerModule.Get().GetProjectCollectionContainer()->TestDynamicQuery(DynamicCollection.Name, DynamicCollection.Type, *this, bUnused);
						--CurrentRecursionDepth;
					}
				}
...
public:
	...
	void SetAsset(FExtAssetFilterTypePtr InAsset)
	{
		...
		if (CollectionManager)
		{
			...
			CollectionManager->GetCollectionContainers(CollectionContainers);
			for (const TSharedPtr<ICollectionContainer>& CollectionContainer : CollectionContainers)
			{
				CollectionContainer->GetCollectionsContainingObject(AssetPtr->GetSoftObjectPath(), ECollectionShareType::CST_All, AssetCollectionNames, ECollectionRecursionFlags::SelfAndChildren);
			}
			...
			for (const FCollectionNameType& DynamicCollection : ReferencedDynamicCollections)
			{
				...
				CollectionManager->GetProjectCollectionContainer()->TestDynamicQuery(DynamicCollection.Name, DynamicCollection.Type, *this, bPassesCollectionFilter);
