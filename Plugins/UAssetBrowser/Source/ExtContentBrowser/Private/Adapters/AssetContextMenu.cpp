...
void FAssetContextMenu::ExecuteRemoveFromCollection()
{
	if ( ensure(SourcesData.Collections.Num() == 1) )
	{
		...
		if ( AssetsToRemove.Num() > 0 )
		{
			...
			CollectionManagerModule.Get().GetProjectCollectionContainer()->RemoveFromCollection(Collection.Name, Collection.Type, AssetsToRemove);
...
bool FAssetContextMenu::CanExecuteShowAssetMetaData() const
{
	...
	{
		if (Asset && FMetaData::GetMapForObject(Asset))
		{
			bResult = true;
			break;
		}
