...
struct FSourcesData
{
	...
	bool IsDynamicCollection() const
	{
		if ( Collections.Num() == 1 && FCollectionManagerModule::IsModuleAvailable() )
		{
			...
			return (CollectionManagerModule.Get().GetProjectCollectionContainer()->GetCollectionStorageMode(
				Collections[0].Name, Collections[0].Type, StorageMode) && StorageMode == ECollectionStorageMode::Dynamic);
		}
	...
	FARFilter MakeFilter(bool bRecurse, bool bUsingFolders) const
	{
		...
		if ( Collections.Num() && FCollectionManagerModule::IsModuleAvailable() )
		{
			...
			for ( int32 CollectionIdx = 0; CollectionIdx < Collections.Num(); ++CollectionIdx )
			{
				...
				CollectionManagerModule.Get().GetProjectCollectionContainer()->GetObjectsInCollection(CollectionNameType.Name, CollectionNameType.Type, ObjectPaths, CollectionRecursionMode);
			}
		}
