...
#include "ExtDependencyViewerSettings.generated.h"

/**
 *  Project based Reference Viewer Saved Settings
 */
USTRUCT()
struct FExtFilterState
{
	GENERATED_BODY();
	...
	FExtFilterState(FTopLevelAssetPath InFilterPath, bool InState)
...
class UExtDependencyViewerSettings : public UObject
{
public:
	GENERATED_BODY()

	UExtDependencyViewerSettings();
	...
	const TArray<FExtFilterState>& GetUserFilters() const;
	void SetUserFilters(TArray<FExtFilterState>& InFilters);

	bool GetFindPathEnabled() const;
	void SetFindPathEnabled(bool bNewEnabled);

	float GetNodeSpacingScale() const;
	void SetNodeSpacingScale(float NewNodeSpacingScale);

	void InitWithDependcyViewerDefaultSetting();
...
private:
	...
	UPROPERTY(config)
	TArray<FExtFilterState> UserFilters;
