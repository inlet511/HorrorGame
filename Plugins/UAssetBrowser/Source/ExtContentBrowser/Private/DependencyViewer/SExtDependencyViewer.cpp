...
void SExtDependencyViewer::Construct(const FArguments& InArgs)
{
	...
	// Default
	// todo: move to FReferenceViewerParams
	Settings->InitWithDependcyViewerDefaultSetting();
	...
	// Fill out CollectionsComboList
	{
		...
		CollectionManagerModule.Get().GetProjectCollectionContainer()->GetCollectionNames(ECollectionShareType::CST_All, CollectionNames);
	...
	// Apply initial node spacing
	SetNodeSpacingScale(Settings->GetNodeSpacingScale());

	// Bind to content browser selection changes
	/*
	FExtContentBrowserModule& ExtContentBrowserModule = FModuleManager::LoadModuleChecked<FExtContentBrowserModule>(TEXT("ExtContentBrowser"));
	FOnExtAssetSelectionChanged& AssetSelectionChangedDelegate = ExtContentBrowserModule.GetOnAssetSelectionChanged();
	AssetSelectionChangedDelegate.AddSP(this, &SExtDependencyViewer::OnSelectedAssetsChanged);
	*/
...
{
	return Settings->GetNodeSpacingScale();
}

void SExtDependencyViewer::SetNodeSpacingScale(float NewValue)
{
	float NewNodeSpacingScale = FMath::Clamp<float>(NewValue, 0.f, 1.f);;

	const float Min = 200.f;
	const float Max = 1200.f;
	float NewNodeXSpacing = Min + (Max - Min) * NewNodeSpacingScale;
	...
	Settings->SetNodeSpacingScale(NewNodeSpacingScale);
}
...
TSharedRef<SWidget> SExtDependencyViewer::GetViewButtonContent()
{
	...
	MenuBuilder.BeginSection("Show", LOCTEXT("ShowHeading", "Show"));
	{
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this] { OnShowHardReferencesChanged(); }),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this] { return IsShowHardReferencesChecked(); })
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this] { OnShowSoftReferencesChanged(); }),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this] { return IsShowSoftReferencesChecked(); })
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SExtDependencyViewer::OnShowCodePackagesChanged),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this] { return IsShowCodePackagesChecked(); })
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		/*
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ShowHideEditorOnlyReferences", "Editor Only References"),
			LOCTEXT("ShowHideEditorOnlyReferencesTooltip", "Show or hide editor only references"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this] { OnShowEditorOnlyReferencesChanged(); }),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this] { return IsShowEditorOnlyReferencesChecked(); })
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		*/
		/*
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ShowHideExternalReferencers", "External Referencers"),
			LOCTEXT("ShowHideExternalReferencersTooltip", "Show or hide external referencers"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this] { OnShowExternalReferencersChanged(); }),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this] { return IsShowExternalReferencersChecked(); })
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		*/
		MenuBuilder.AddMenuEntry(
			LOCTEXT("CompactMode", "Compact Mode"),
			LOCTEXT("CompactModeTooltip", "Show nodes in compact mode without thumbnails"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this] { OnCompactModeChanged(); }),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this] { return IsCompactModeChecked(); })
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		/*
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ShowFilteredPackagesOnly", "Show Filtered Packages Only"),
			LOCTEXT("ShowFilteredPackagesOnlyTooltip", "Show only filtered packages"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this] { OnShowFilteredPackagesOnlyChanged(); }),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this] { return IsShowFilteredPackagesOnlyChecked(); })
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
...
void SExtDependencyViewer::OnSelectedAssetsChanged(const TArray<FExtAssetData>& NewSelectedAssets, bool bIsPrimaryBrowser)
{
	if (NewSelectedAssets.Num() > 0)
	{
		// Convert selected assets to identifiers
		TArray<FExtAssetIdentifier> AssetIdentifiers;
		FExtAssetRegistry::ExtractAssetIdentifiersFromAssetDataList(NewSelectedAssets, AssetIdentifiers);
...
void SExtDependencyViewer::OnShowSoftReferencesChanged()
{
	Settings->SetShowSoftReferencesEnabled(!Settings->IsShowSoftReferences());
	RebuildGraph();
}

bool SExtDependencyViewer::IsShowSoftReferencesChecked() const
{
	return Settings->IsShowSoftReferences();
}

void SExtDependencyViewer::OnShowHardReferencesChanged()
{
	Settings->SetShowHardReferencesEnabled(!Settings->IsShowHardReferences());
	RebuildGraph();
}

bool SExtDependencyViewer::IsShowHardReferencesChecked() const
{
	return Settings->IsShowHardReferences();
}

bool SExtDependencyViewer::GetManagementReferencesVisibility() const
{
	return bShowShowReferencesOptions;
}

void SExtDependencyViewer::OnShowManagementReferencesChanged()
{
...
bool SExtDependencyViewer::IsShowManagementReferencesChecked() const
{
	return Settings->IsShowManagementReferences();
}

void SExtDependencyViewer::OnShowSearchableNamesChanged()
{
	Settings->SetShowSearchableNames(!Settings->IsShowSearchableNames());
	RebuildGraph();
}

bool SExtDependencyViewer::IsShowSearchableNamesChecked() const
{
	return Settings->IsShowSearchableNames();
}

void SExtDependencyViewer::OnShowCodePackagesChanged()
{
	Settings->SetShowCodePackages(!Settings->IsShowCodePackages());
	RebuildGraph();
}

bool SExtDependencyViewer::IsShowCodePackagesChecked() const
{
	return Settings->IsShowCodePackages();
}

void SExtDependencyViewer::OnShowEditorOnlyReferencesChanged()
{
...
bool SExtDependencyViewer::IsShowEditorOnlyReferencesChecked() const
{
	return Settings->IsShowEditorOnlyReferences();
}

void SExtDependencyViewer::OnShowExternalReferencersChanged()
{
	// This can take a few seconds if it isn't ready
	//UAssetManager::Get().UpdateManagementDatabase(); 
	Settings->SetShowManagementReferencesEnabled(!Settings->IsShowManagementReferences());
...
bool SExtDependencyViewer::IsShowExternalReferencersChecked() const
{
	return Settings->IsShowManagementReferences();
}

void SExtDependencyViewer::OnCompactModeChanged()
{
	Settings->SetCompactModeEnabled(!Settings->IsCompactMode());
	RebuildGraph();
}

bool SExtDependencyViewer::IsCompactModeChecked() const
{
	return Settings->IsCompactMode();
}

void SExtDependencyViewer::OnShowFilteredPackagesOnlyChanged()
{
	Settings->SetShowExternalReferencersEnabled(!Settings->IsShowExternalReferencers());
	RebuildGraph();
}

bool SExtDependencyViewer::IsShowFilteredPackagesOnlyChecked() const
{
	return Settings->IsShowExternalReferencers();
}

void SExtDependencyViewer::UpdateIsPassingSearchFilterCallback()
{
	/*
	if (GraphObj)
	{
...
FString SExtDependencyViewer::GetReferencedObjectsList() const
{
	...
	if (AllSelectedPackageNames.Num() > 0)
	{
		...
		for (const FName SelectedPackageName : AllSelectedPackageNames)
		{
			...
			FExtContentBrowserSingleton::GetAssetRegistry().GetDependencies(SelectedPackageName, HardDependencies, EDependencyCategory::Package, FDependencyQuery(EDependencyQuery::Hard));
			
			TArray<FName> SoftDependencies;
			FExtContentBrowserSingleton::GetAssetRegistry().GetDependencies(SelectedPackageName, SoftDependencies, EDependencyCategory::Package, FDependencyQuery(EDependencyQuery::Soft));
...
FString SExtDependencyViewer::GetReferencingObjectsList() const
{
	...
	if (AllSelectedPackageNames.Num() > 0)
	{
		...
		{
			TArray<FName> HardDependencies;
			FExtContentBrowserSingleton::GetAssetRegistry().GetReferencers(SelectedPackageName, HardDependencies, UE::AssetRegistry::EDependencyCategory::Package, UE::AssetRegistry::EDependencyQuery::Hard);
...
void SExtDependencyViewer::MakeCollectionWithReferencersOrDependencies(ECollectionShareType::Type ShareType, bool bReferencers)
{
	...
	if (AllSelectedPackageNames.Num() > 0)
	{
		if (ensure(ShareType != ECollectionShareType::CST_All))
		{
			...
			CollectionManagerModule.Get().GetProjectCollectionContainer()->CreateUniqueCollectionName(*CollectionNameAsText.ToString(), ShareType, CollectionName);
