...
void ResetLoaders(const FString& InPackageName)
{
	...
	UPackage* ExistingPackage = FindObject<UPackage>(nullptr, *InPackageName, EFindObjectFlags::ExactClass);
	if (ExistingPackage != nullptr)
	{
		...
		ExistingPackage->GetMetaData().RemoveMetaDataOutsidePackage(ExistingPackage);
	}
}
...
void FExtPackageUtils::GeneratePackageReport(FLinkerLoad* Linker, uint32 InfoFlags, bool bHideOffsets)
{
	...
	{
		ECB_LOG(Display, TEXT("\t             Guid: %s"), *LexToString(Linker->Summary.GetSavedHash()));
	}
